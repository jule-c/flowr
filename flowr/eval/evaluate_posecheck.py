...
from flowr.util.metrics import evaluate_posecheck
...
def _calc_posecheck_metrics(
    ...
    # PoseCheck
    posecheck_metrics = evaluate_posecheck(
        gen_ligs, pdb_file=ref_pdb, return_list=return_list
    )
    return posecheck_metrics


def _calc_posecheck_metrics_single(args):
    """
    ...
    pc_results = _calc_posecheck_metrics(
        gen_ligs, str(ref_pdb), return_list=return_list
    )
    return {f"{Path(ref_pdb).stem}": pc_results}


def calc_posecheck_metrics_parallel(
    ...
    with multiprocessing.Pool(processes=num_workers) as pool:
        results = pool.map(_calc_posecheck_metrics_single, worker_args)
    ...
    # Compute the final dict that only contains PC metrics mean and mean std aggregated over all targets
...
if __name__ == "__main__":
    ...
    print(f"Computing PoseCheck metrics on {args.num_workers} CPUs...")
    print(
        f"Computing PoseCheck metrics for {len(list(itertools.chain(*gen_ligs)))} generated molecules..."
    )
    ...
    posecheck_metrics = calc_posecheck_metrics_parallel(
        gen_ligs_san,
        ref_pdbs,
    ...
    if args.evaluate_test_data or args.evaluate_val_data or args.evaluate_train_data:
        ...
        save_path = os.path.join(args.save_dir, f"posecheck_metrics_dict_{state}.pt")
    else:
        save_path = os.path.join(args.save_dir, "posecheck_metrics_dict.pt")
    torch.save(posecheck_metrics, save_path)
    if not args.return_list:
        print(f"PoseCheck metrics: {posecheck_metrics}")
    print(f"PoseCheck metrics saved to {save_path}")
