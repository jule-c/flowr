...
from flowr.util.metrics import evaluate_clashes
...
def _calc_clashes_metrics(
    ...
    # Clash count
    clashes_metrics = evaluate_clashes(gen_ligs, ref_pdb, return_list=return_list)
    return clashes_metrics


def _calc_clashes_metrics_single(args):
    """
    ...
    clashes_results = _calc_clashes_metrics(
        gen_ligs, str(ref_pdb), return_list=return_list
    )
    return {f"{Path(ref_pdb).stem}": clashes_results}


def calc_clashes_metrics_parallel(
    ...
    """
    Computes Clash count for each target in parallel using one task per target.
    """
    ...
    with multiprocessing.Pool(processes=num_workers) as pool:
        results = pool.map(_calc_clashes_metrics_single, worker_args)
...
if __name__ == "__main__":
    ...
    print(f"Computing Clash count metrics on {args.num_workers} CPUs...")
    print(
        f"Computing Clash count metrics for {len(list(itertools.chain(*gen_ligs)))} generated molecules..."
    )
    ...
    clashes_metrics = calc_clashes_metrics_parallel(
        gen_ligs_san,
        ref_pdbs,
    ...
    if args.evaluate_test_data or args.evaluate_val_data or args.evaluate_train_data:
        ...
        save_path = os.path.join(args.save_dir, f"clashes_metrics_dict_{state}.pt")
    else:
        save_path = os.path.join(args.save_dir, "clashes_metrics_dict.pt")
    torch.save(clashes_metrics, save_path)
    if not args.return_list:
        print(f"Clash count metrics: {clashes_metrics}")
    print(f"Clash count metrics saved to {save_path}")
