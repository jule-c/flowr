...
Note that the script currently does not save the molecules in batches - all of the molecules are generated and then saved. If generating many molecules ensure you have enough memory to store them.
"""
...
from callbacks.prediction_writer import PredictionWriter
...
def load_model(args):
    ...
    return fm_model, hparams, dataset_info, vocab, vocab_pocket_atoms, vocab_pocket_res
...
def build_dm(args, hparams, vocab, dataset_info):
    ...
    if args.dataset in ["qm9", "geom-drugs"]:
        ...
        eval_interpolant = ComplexInterpolant(
            ...
            ligand_type_interpolation=hparams["integration-type-strategy"],
            ligand_bond_interpolation=hparams["integration-bond-strategy"],
...
def main(args):
    print(
        f"Running prediction script for {args.sample_n_molecules_per_target} molecules per target..."
    )
    ...
    dataloader = (
        dm.test_dataloader() if args.dataset_split == "test" else dm.val_dataloader()
    )
    ...
    strategy = DDPStrategy(timeout=datetime.timedelta(seconds=1800 * args.gpus))
    pred_writer = PredictionWriter(output_dir=args.save_dir, write_interval="epoch")
    trainer = pl.Trainer(
        accelerator="gpu",
        strategy=strategy,
        devices=args.gpus,
    ...
    L.seed_everything(args.seed, workers=args.gpus > 1)
