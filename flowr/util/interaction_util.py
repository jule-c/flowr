...
class InteractionFingerprints:
    ...
    def __call__(
        ...
        if return_array:
            if isinstance(ligand_mol, list):
                ...
                # return [
                #     self.interaction_array(plf_fp, ifp_idx=i)
                #     for i in range(len(ligand_mol))
                # ]
            else:
                self.n_pocket, self.n_lig = (
                    pocket_mol.GetNumAtoms(),
                    ligand_mol.GetNumAtoms(),
                )
                return self.interaction_array(plf_fp)
        else:
            return plf_fp
    ...
    def interaction_array(
        ...
        arr_shape = (self.n_pocket, self.n_lig, len(self.interaction_types))
