...
class ProteinPocket:
    ...
    def res_names(self):
        res_names = torch.tensor(
            [res_name_encoder_plinder[res] for res in self.atoms.res_name]
        ).long()
        return res_names
    ...
    def atom_names(self):
        ...
        atom_names = torch.tensor(
            [
                atom_name_encoder_plinder[fix_atom_name(atom)]
    ...
    #     atom_names = torch.tensor(
    #         [atom_name_encoder_plinder[atom] for atom in self.atoms.atom_name]
    #     ).long()  # .replace("'", ""))]
    #     return atom_names
    ...
    def to_geometric_mol(self) -> GeometricMol:
        ...
        ).long()  # TODO remove this when we have a better way to handle bond types. biotite provides much more sophisticated bond types, like aromatic single, double etc. For now, we just map all aromatic bond types to 4 (aromatic)
...
class PocketComplex:
    ...
    @staticmethod
    def from_bytes(data: bytes, remove_hs: bool = False) -> PocketComplex:
        obj = pickle.loads(data)
    ...
    def remove_hs(self, include_ligand: Optional[bool] = False) -> PocketComplex:
    ...
    def move_apo_and_holo_and_lig_to_apo_com(self):
        ...
        ligand = self.ligand
        holo_mol, apo_mol = self.apo.mol, self.holo.mol
...
class PocketComplexBatch(Sequence):
    ...
    @staticmethod
    def from_bytes(data: bytes, remove_hs: bool = False) -> PocketComplexBatch:
        systems = [
            PocketComplex.from_bytes(system, remove_hs=remove_hs)
