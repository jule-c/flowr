...
from genbench3d.geometry import ClashChecker, GeometryExtractor, ReferenceGeometry
...
class Validity3D(Metric):
    ...
    def __init__(
        ...
        self.clash_checker = ClashChecker(
            safety_ratio=steric_clash_safety_ratio, consider_hs=self.consider_hs
        )
    ...
    def get(self, cel: GeneratedCEL, average=True) -> float:
        ...
        for name, ce in tqdm(cel.items()):
            ...
            mol = ce.mol
            valid_conf_ids = self.get_valid_conf_ids_for_mol(mol, name)
        ...
        if average:
            ...
            self.value = self.n_valid_confs

        return self.value

    def get_valid_conf_ids_for_mol(
        self,
        mol: Mol,
        name: str = None,
        ...
        valid_bond_conf_ids, bond_validities, new_bond_patterns = self.analyze_bonds(
            mol
        )
        ...
        valid_angle_conf_ids, angle_validities, new_angle_patterns = (
            self.analyze_angles(mol)
        )
        ...
        if analyze_torsions:  # this is slower than bonds and angles
            valid_torsion_conf_ids, torsion_validities, new_torsion_patterns = (
                self.analyze_torsions(mol)
            )
    ...
    def analyze_bonds(
        ...
        bonds = self.geometry_extractor.get_bonds(
            mol, consider_hydrogens=self.consider_hs
        )
        ...
        for conf in mol.GetConformers():
            ...
            for bond in bonds:
                ...
                q_value, new_pattern = self.reference_geometry.geometry_is_valid(
                    geometry_pattern=bond_pattern, value=bond_value, geometry="bond"
                )
    ...
    def analyze_angles(
        ...
        angles_atom_ids = self.geometry_extractor.get_angles_atom_ids(
            mol, consider_hydrogens=self.consider_hs
        )
        ...
        for conf in mol.GetConformers():
            ...
            for begin_atom_idx, second_atom_idx, end_atom_idx in angles_atom_ids:
                ...
                q_value, new_pattern = self.reference_geometry.geometry_is_valid(
                    geometry_pattern=angle_pattern, value=angle_value, geometry="angle"
                )
    ...
    def analyze_torsions(
        ...
        torsions_atom_ids = self.geometry_extractor.get_torsions_atom_ids(
            mol, consider_hydrogens=self.consider_hs
        )
        ...
        for conf in mol.GetConformers():
            ...
            for (
                ...
                q_value, new_pattern = self.reference_geometry.geometry_is_valid(
                    geometry_pattern=torsion_pattern,
                    value=torsion_value,
                    geometry="torsion",
                )
    ...
    def analyze_rings(self, mol: Mol) -> tuple[list[int], dict[str, Any]]:
    ...
    def get_max_distance_to_plane(self, conf: Conformer, ring_atom_ids: list[int]):
    ...
    def analyze_clashes(self, mol: Mol) -> Tuple[List[int], Dict[str, Any]]:
        ...
        try:
            ...
            if not self.consider_hs:
                mol = Chem.RemoveHs(
                    mol
                )  # Saves time by not computing H interatomic distances
            ...
            bond_idxs = [t if t[0] < t[1] else (t[1], t[0]) for t in bond_idxs]
            angle_idxs = self.geometry_extractor.get_angles_atom_ids(
                mol, consider_hydrogens=self.consider_hs
            )
            two_hop_idxs = [
                (t[0], t[2]) if t[0] < t[2] else (t[2], t[0]) for t in angle_idxs
            ]
            torsion_idxs = self.geometry_extractor.get_torsions_atom_ids(
                mol, consider_hydrogens=self.consider_hs
            )
            three_hop_idxs = [
                (t[0], t[3]) if t[0] < t[3] else (t[3], t[0]) for t in torsion_idxs
            ]
    ...
    def get_invalid_geometries(self):
        ...
        for name, validity_list in self.validities.items():
            ...
            if len(validity_df) > 0:
                invalidity_df = validity_df[
                    validity_df["q-value"] < self.q_value_threshold
                ].copy()
                invalidity_df["name"] = name
                if invalidity_df.shape[0] > 0:
                    invalid_numbers_df = invalidity_df.pivot_table(
                        values="value",
                        index=["name", "conf_id"],
            ...
            if len(validity_df) > 0:
                ...
                if invalidity_df.shape[0] > 0:
                    ...
                    if "bond" in invalid_numbers_df.columns:
                        all_n_inv_bonds.extend(
                            invalid_numbers_df["bond"].values.tolist()
                        )
                    if "angle" in invalid_numbers_df.columns:
                        all_n_inv_angles.extend(
                            invalid_numbers_df["angle"].values.tolist()
                        )
    ...
    def get_min_q_values_for_geometry(self, geometry_type: str = "bond"):
        return self.get_agg_q_values_for_geometries(
            agg_func=np.min, geometry_types=[geometry_type]
        )

    def get_geo_mean_q_values_for_geometry(self, geometry_type: str = "bond"):
        return self.get_agg_q_values_for_geometries(
            agg_func=gmean, geometry_types=[geometry_type]
        )

    def get_min_q_values_for_geometries(
        self, geometry_types: list[str] = ["bond", "angle", "torsion"]
    ):
        return self.get_agg_q_values_for_geometries(
            agg_func=np.min, geometry_types=geometry_types
        )

    def get_geo_mean_q_values_for_geometries(
        self, geometry_types: list[str] = ["bond", "angle", "torsion"]
    ):
        return self.get_agg_q_values_for_geometries(
            agg_func=gmean, geometry_types=geometry_types
        )

    def get_agg_q_values_for_geometries(
        self,
        agg_func: callable,
        ...
        for name, validity_list in self.validities.items():
            validity_df = pd.DataFrame(validity_list)
            if len(validity_df) > 0:
                geometry_validity_df = validity_df[
                    validity_df["geometry_type"].isin(geometry_types)
                ]
                geometry_validity_df = geometry_validity_df.dropna(subset=["q-value"])
                if geometry_validity_df.shape[0] > 1:
                    try:
                        q_value_series = geometry_validity_df.groupby("conf_id")[
                            "q-value"
                        ].agg(agg_func)
                    except Exception as e:
                        print(e)
                    ...
                    # import pdb;pdb.set_trace()
                    agg_q_values.extend(
                        [float("nan")] * len(validity_df["conf_id"].unique())
                    )
