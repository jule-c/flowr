...
class FilterAtomTypes(FilterFunc):
    ...
    def __init__(self, allowed_atoms: list[str]):
        ...
        func_desc = (
            "Removes systems which have atom types which are not in the allowed set"
        )
        super().__init__(func_repr, func_desc)
    ...
    def filter(self, system: PocketComplex) -> bool:
        ligand_symbols = [
            PT.symbol_from_atomic(atom) for atom in system.ligand.atomics.tolist()
        ]
        is_allowed_ligand = [
            symbol in self.allowed_atom_set for symbol in ligand_symbols
        ]
        is_allowed_holo = [
            symbol in self.allowed_atom_set for symbol in system.holo.atom_symbols
        ]
...
class FilterLigands(FilterFunc):
    ...
    def __init__(self, smiles: list[str]):
        ...
        all_atom_symbols = [
            PT.symbol_from_atomic(atomic) for atomic in range(MAX_ATOMIC_NUM + 1)
        ]
        self.smiles_set = set([self._to_canonical(smi) for smi in smiles])
        self.vocab = Vocabulary(all_atom_symbols)

    def filter(self, system: PocketComplex) -> bool:
        smiles = smolRD.smiles_from_mol(
            system.ligand.to_rdkit(self.vocab), canonical=True
        )
        return smiles in self.smiles_set

    def _to_canonical(self, smiles: str) -> str:
        return smolRD.smiles_from_mol(
            smolRD.mol_from_smiles(smiles), canonical=True, explicit_hs=False
        )
...
def save_splits_(args, systems: PocketComplexBatch):
    ...
    for system in systems:
        if system.split == "train":
            ...
            raise ValueError(
                f"Unknown split for system {system.system_id}, got split {system.split}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_filter_covalent", action="store_false", dest="filter_covalent"
    )
    parser.add_argument(
        "--no_filter_atom_types", action="store_false", dest="filter_atom_types"
    )
    parser.add_argument(
        "--no_filter_bad_ligand", action="store_false", dest="filter_bad_ligand"
    )
    parser.add_argument(
        "--filter_min_residues", type=int, default=DEFAULT_FILTER_MIN_RESIDUES
    )

    parser.set_defaults(
        filter_covalent=True, filter_atom_types=True, fiter_bad_ligand=True
    )
